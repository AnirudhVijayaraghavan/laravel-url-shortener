...
export default class Search {
  ...
  async sendRequest() {
    const results = await axios(`/homepage/search/${this.inputField.value}`)
    this.renderResultsHTML(results.data)
  }

  renderResultsHTML(urls) {
    if (urls.length) {
      this.resultsArea.innerHTML = DOMPurify.sanitize(`<div class="list-group shadow-sm">
      <div class="list-group-item active"><strong>Search Results</strong> (${urls.length > 1 ? `${urls.length} items found` : "1 item found"})</div>
      ${urls
        .map(url => {
          let urlDate = new Date(url.created_at)
          return `<a href="/${url.shortURL}" class="list-group-item list-group-item-action">
        <img class="avatar-tiny" src="${url.user.avatar}">Label: ${url.label}, Original URL: <strong>${url.longURL}</strong>, Shortened URL: <strong>${url.shortURL}</strong>
        <span class="text-muted small">by ${url.user.username} on ${urlDate.getMonth() + 1}/${urlDate.getDate()}/${urlDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
